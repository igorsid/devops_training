...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "server1" do |server1|
    ...
    server1.vm.network "private_network", ip: "192.168.10.11"
    ...
    server1.vm.provision "shell", inline: <<-SHELL
      sudo -u vagrant sh -c "git clone https://github.com/igorsid/module2.git"
      sudo -u vagrant sh -c "cd ~/module2; git checkout task2; cat some_text_file.txt"
    SHELL
  ...
  config.vm.define "server2" do |server2|
    ...
    server2.vm.host_name = "server2.localdomain"
    server2.vm.network "private_network", ip: "192.168.10.12"
